...
def _create_optimizer(conf, model):
    ...
    optimizer = torch.optim.AdamW(filter(
        ...
        betas=betas, lr=learning_rate, weight_decay=weight_decay)
    return optimizer
...
def evaluation(pre, mask, cutoff, file, min_size=10):
    ...
    for i in range(len(pre)):
            ...
            PQ = np.append(PQ, metrics.get_fast_pq(
                np.array(mask[i], dtype='uint8'),
                np.array(img, dtype='uint8'))[0][2])

    my_print('Num is:{} '.format(len(PQ)),
             'cutoff=[{}]'.format(cutoff),
             'PQ=[{:.6}]'.format(np.mean(PQ)),
    ...
    def predition(self, img, HE, file):
        with torch.no_grad():
            ...
            io.imsave(os.path.join(
                self.conf.save_path, file[0:(len(file) - 4)] + '-pre.png'),
                      np.array(
                (nuclei > self.conf.cutoff) * 255, dtype='uint8'))

            io.imsave(os.path.join(
                self.conf.save_path, file[0:(len(file) - 4)] + '-RGB.png'),
                np.array(RGB * 255, dtype='uint8'))

            io.imsave(os.path.join(
                self.conf.save_path, file[0:(len(file) - 4)] + '-H.png'),
                      np.array(H * 255, dtype='uint8'))
...
#              transform.resize((256+128,256+128)),
        ...
        self.train_data_Generator = self.data_Generator(
            self.conf.train_data_path, self.conf.label_path,
            self.conf.edg_path, data_transform)
    ...
    def training(self):
        ...
        for epoch in range(self.conf.epoches):
            my_print('Epoch {}/{}'.format(epoch, self.conf.epoches - 1),
                     '-' * 60)
            start = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', default=1, required=False,
                        type=int, help='1:trian or 0:test')
